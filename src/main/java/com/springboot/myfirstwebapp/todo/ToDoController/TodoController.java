...
// @Controller //if we comment this out then spring wont know its a
// controller and other controller will be used @SessionAttributes("name") // needs to be mentioned in places where its used
...
public class TodoController {
     ...
     * in jsp and thats why if we put values in the above field we also see them
     * in
     * the
     * form as default
     */
    ...
    public String showNewTodoPage(ModelMap model) {
        Todo todo = new Todo(0, "default value", "this value is taken from the mapped method in controller",
     ...
     * and that form gets a data as POST method and then this method will be
     * invoked
     ...
     * are going to map the coming data to bean of the Todo class and todo class
     * obj
     * will get the value from the form to use that we have made some changes in
     * the
     * todo jsp
     ...
     * public String addNewTodo(@RequestParam String username, @RequestParam
     * String
    ...
    public String deleteTodo(@RequestParam int id) {
        ...
        // from the list of todos, but since we are not calling that method and going in
        // the HTML page, we are not
        // giving the form the attribute details it needs to show data hence its not
        // showing so we must return the list to method and for that we need to redirect
        return "redirect:list-todos";
    }
    ...
    public String updateTodo(@Valid Todo todo, BindingResult res) {
        if (res.hasErrors()) {
